...
import { FileText, FileSpreadsheet, Download, Calendar, Filter, PieChart, BarChart3, ArrowUpRight, ArrowDownLeft } from 'lucide-react';

export const ReportsView: React.FC = () => {
  const { deliveries, expenses, udhaarCustomers, workers, dailySalesEntries } = useApp();
  ...
  const totalUdhaarOutstanding = udhaarCustomers.reduce((acc, curr) => acc + curr.remainingBalance, 0);
  const netProfit = totalDailySalesAmount - totalDeliveriesCost - totalExpensesAmount;

  const handleExportPDF = () => {
    if (reportType === 'DAILY_SALES') {
      const headers = ['Date', 'Business Section', 'Total Daily Sales', 'Recorded By', 'Notes'];
      const rows = (dailySalesEntries || []).map(e => [
        e.date,
        e.section,
      ...
      exportToPDF('Bahu Petroleum - Daily Cash & Card Sales Report', headers, rows, 'Daily_Sales_Report');
    } else if (reportType === 'DELIVERIES') {
      const headers = ['Date', 'Supplier', 'Invoice', 'Fuel Type', 'Quantity', 'Amount'];
      ...
      exportToPDF('Bahu Petroleum - Fuel Deliveries Report', headers, rows, 'Fuel_Deliveries_Report');
    } else {
      const headers = ['Expense Title', 'Category', 'Date', 'Amount'];
      const rows = expenses.map(e => [e.title, e.category, e.date, `Rs.${e.amount}`]);
      exportToPDF('Bahu Petroleum - Expenses Statement', headers, rows, 'Expenses_Report');
    }
  };
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-bold text-xs sm:text-sm flex items-center gap-1.5 shadow-md transition-all"
          >
            <FileText className="w-4 h-4" /> Download PDF Report
          </button>
          <button
            onClick={handleExportExcel}
            className="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs sm:text-sm flex items-center gap-1.5 shadow-md transition-all"
          >
